...
class ElementSpec(TypedDict):
    element_id: str
    element_name: Optional[str]    # None when spec DB omitted; BYOS (Bring Your Own Spec) model keeps OSS wheel copyright-clean (ref: DL-003)
    data_type: str                 # "AN", "ID", "DT", "TM", "R", "N0" per X12 data type table
    min_length: int
    max_length: int
    requirement: str               # "M"=mandatory, "O"=optional, "C"=conditional, "X"=excluded
    sequence: int                  # 1-based position within parent segment; frozen public API from v1.2.0 (ref: DL-003)
...
class SegmentSpec(TypedDict):
    ...
    loop_repeat: int               # Maximum loop repetitions; used by Phase 3 transformer for hierarchy reconstruction
...
class X12SpecProvider(Protocol):
    ...
    def get_element_codes(
        self, version: str, element_id: str
    ) -> list[str]: ...

    def get_segment_spec(
        self, version: str, segment_id: str
